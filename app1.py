...
class FlashcardApp:
    def __init__(self, root):
        ...
        image_path = "D:/app/propic.jpg"  # Update this path to the correct location of your background image
        ...
        # Admin Block
        admin_frame = tk.LabelFrame(root, text="Admin Block: Deck Management", padx=40, pady=10)
        self.canvas.create_window(root.winfo_screenwidth()//2, root.winfo_screenheight() * 0.05, window=admin_frame, anchor="n")

        button_row_frame = tk.Frame(admin_frame)
        button_row_frame.pack(pady=3)
        ...
        tk.Button(admin_frame, text="Refresh", command=self.show_csv_files, bg="black", fg="white", width=20).pack(pady=3)

        # User Block with spacing adjustment
        user_frame = tk.LabelFrame(root, text="User Block: Select and Load Deck", padx=10, pady=10)
        self.canvas.create_window(root.winfo_screenwidth() // 2, root.winfo_screenheight() * 0.2, window=user_frame, anchor="n")
        ...
        self.deck_menu.grid(row=0, column=0, padx=5, pady=5, sticky="w")

        load_button = tk.Button(user_frame, text="Load Deck", command=self.load_deck, bg="grey", fg="white", width=20)
        load_button.grid(row=0, column=1, padx=10, pady=5, sticky="e")

        # Review Mode Block with additional spacing
        review_frame = tk.LabelFrame(root, text="Review Mode", padx=10, pady=10)
        self.canvas.create_window(root.winfo_screenwidth()//2, root.winfo_screenheight() * 0.35, window=review_frame, anchor="n")

        self.question_label = tk.Label(review_frame, text="Question:", font=("Helvetica", 14, "bold"))
        self.question_label.grid(row=0, column=0, padx=5, pady=2, sticky="w")

        self.question_box = tk.Text(review_frame, wrap="word", height=4, width=40, font=("Helvetica", 12))
        self.question_box.grid(row=1, column=1, columnspan=2, padx=5, pady=2, sticky="ew")
        self.question_box.config(state="disabled")

        self.answer_label = tk.Label(review_frame, text="Answer:", font=("Helvetica", 14, "bold"))
        self.answer_label.grid(row=2, column=0, padx=5, pady=2, sticky="w")

        self.answer_box = tk.Text(review_frame, wrap="word", height=4, width=40, font=("Helvetica", 12), fg="dark green")
        self.answer_box.grid(row=3, column=1, columnspan=2, padx=5, pady=2, sticky="ew")
        self.answer_box.config(state="disabled")

        tk.Button(review_frame, text="Show Answer", command=self.show_answer, bg="black", fg="white").grid(row=4, column=1, pady=2, padx=(0,200), sticky="ew")
        tk.Button(review_frame, text="Next Card", command=self.next_card, bg="orange", fg="white").grid(row=4, column=1, pady=2, padx=(200,0), sticky="ew")
        tk.Button(review_frame, text="Previous Card", command=self.previous_card, bg="grey", fg="white").grid(row=5, column=1, pady=2, sticky="ew")

        self.stats_label = tk.Label(review_frame, text=" ", font=("Arial", 10))
        self.stats_label.grid(row=6, column=0, columnspan=2, pady=2, sticky="ew")
